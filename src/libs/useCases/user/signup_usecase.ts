...
export const signup_usecase=(dependencies:any)=>{
    ...
    const execute = async(first_name: string , last_name: string , email: string , phone: string , password: string)=>{
        await userRepository.validate_signup({first_name,last_name,email,phone,password})
        let userExist = await userRepository.findUserByEmail(email);
        if(userExist){
            throw new Error("User already exist")
        }else{
            let newUser = await userRepository.insertUser(first_name,last_name,email,phone,password)
